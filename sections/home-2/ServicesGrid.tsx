  ...
  return (
    <section id="services" className="py-24 bg-white scroll-mt-24">
      <div className="mx-auto max-w-6xl px-6">
        ...
        {/* Equal-weight capability index — one row, divided columns */}
        <AnimatedSection>
          <div className="mt-14 grid grid-cols-1 overflow-hidden rounded-3xl border border-border bg-white shadow-sm sm:grid-cols-2 lg:grid-cols-4">
            {pillars.map((p) => (
              <Link
                key={p.title}
                href={p.href}
                className="group relative isolate flex flex-col overflow-hidden p-8 transition-colors duration-300 hover:bg-accent sm:[&:nth-child(odd)]:border-r sm:[&:nth-child(odd)]:border-border sm:[&:nth-child(-n+2)]:border-b sm:[&:nth-child(-n+2)]:border-border lg:border-b-0 lg:border-r lg:border-border lg:last:border-r-0"
              >
